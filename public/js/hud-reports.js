      ...
      <table class="status-table">
        ...
        <tbody>
          ${data.counselors.filter(c => c.active !== false).map(c => {
            const myHud = data.hudEntries.filter(e => e.counselorId === c.id);
            const myTal = data.talEntries.filter(e => e.counselorId === c.id);
            const parDays = new Set(myHud.filter(e => (e.section||'').startsWith('PAR')).map(e => e.date)).size;
            const cmlCnt  = myHud.filter(e => e.section === 'CML').length;
            const talCnt  = myTal.length;
            const issues  = [];
...
async function loadAllData(year, mon) {
  ...
  const [counselorSnap, clientSnap, rxSnap, sessSnap, hudSnap, talSnap] = await Promise.all([
    ...
    getDocs(query(collection(db, 'hudTimeEntries'), where('month', '==', monthKey))),
    getDocs(query(collection(db, 'hudTrainingEntries'), where('month', '==', monthKey))),
  ]);
  ...
  // NOFA active Rx map: clientId → first NOFA rxNumber
  const nofaRxMap = {};
  rxSnap.docs.forEach(d => {
    const r        = d.data();
    const clientId = d.ref.parent.parent.id;
    if (r.guarantor === 'NOFA' && r.active !== false && !nofaRxMap[clientId]) {
      nofaRxMap[clientId] = r.rxNumber || '';
    }
  });
  ...
  const hudEntries = hudSnap.docs.map(d => ({ id: d.id, ...d.data() }));
  const talEntries = talSnap.docs.map(d => ({ id: d.id, ...d.data() }));

  return { year, mon, ld, lastDayFmt: fmtDate(ld), fileDateStr: fileDate(year, mon),
           counselors, clientMap, nofaRxMap, sessions, hudEntries, talEntries };
}
...
async function genCAL(data) {
  const { year, mon, lastDayFmt, fileDateStr, counselors, clientMap, nofaRxMap, sessions, hudEntries } = data;

  // Qualifying sessions: In-Person client with NOFA Rx
  const rows = sessions
    .filter(s => {
      const c = clientMap[s.clientId];
      return c && c.billingType === 'In-Person' && nofaRxMap[s.clientId];
    })
  ...
  rows.forEach((s, i) => {
    const c       = clientMap[s.clientId];
    const rxNum   = nofaRxMap[s.clientId] || '';
    const minutes = roundTo15((parseFloat(s.hours) || 0) * 60);
    const r       = dataStartRow + 1 + i;
    ws.getCell(r, 1).value = fmtDate(s.dateObj);
    ws.getCell(r, 2).value = rxNum;
    ws.getCell(r, 3).value = s.counselor || '';
    ws.getCell(r, 4).value = c.firstName;
    ws.getCell(r, 5).value = c.lastName;
    ws.getCell(r, 6).value = 'In-Person';
    ws.getCell(r, 7).value = 'Under Progress';
    ws.getCell(r, 8).value = minutes;
  });
...
async function buildPAR(counselor, data) {
  const { year, mon, ld, lastDayFmt, fileDateStr, clientMap, nofaRxMap, sessions, hudEntries, talEntries } = data;
  const daysInMonth = ld.getDate();

  // Filter hudTimeEntries for this counselor
  const myHud = hudEntries.filter(e => e.counselorId === counselor.id);
  const myTal = talEntries.filter(e => e.counselorId === counselor.id);

  // Build day totals for each S1 row from hudTimeEntries
  // Counseling row comes from In-Person sessions directly
  const s1DayTotals = {}; // parRow → { day: hours }
  Object.keys(S1_ROWS).forEach(row => { s1DayTotals[row] = {}; });

  myHud
    .filter(e => e.section === 'PAR-S1' && e.parRow !== 'Counseling')
    .forEach(e => {
      const day = parseInt((e.date || '').split('-')[2], 10);
      if (!day) return;
      s1DayTotals[e.parRow] = s1DayTotals[e.parRow] || {};
      s1DayTotals[e.parRow][day] = (s1DayTotals[e.parRow][day] || 0) + (e.hours || 0);
    });

  // Counseling = In-Person sessions for this counselor
  const counselingDays = {};
  sessions
    .filter(s => s.counselor === counselor.name && clientMap[s.clientId]?.billingType === 'In-Person')
    .forEach(s => {
      const day = parseInt((s.date || '').split('-')[2], 10) || s.dateObj.getDate();
      counselingDays[day] = (counselingDays[day] || 0) + (parseFloat(s.hours) || 0);
    });
  s1DayTotals['Counseling'] = counselingDays;

  // Section 2: Training — from hudTimeEntries section=PAR-S2
  const s2Days = {};
  myHud.filter(e => e.section === 'PAR-S2').forEach(e => {
    const day = parseInt((e.date || '').split('-')[2], 10);
    if (!day) return;
    s2Days[day] = (s2Days[day] || 0) + (e.hours || 0);
  });
  // Also pull from hudTrainingEntries (T = training)
  myTal.filter(e => e.certificationActivityType === 'T').forEach(e => {
    const day = parseInt((e.date || '').split('-')[2], 10);
    if (!day) return;
    s2Days[day] = (s2Days[day] || 0) + (parseFloat(e.durationHours || 0));
  });

  // Section 3: Marketing — from hudTimeEntries section=PAR-S3
  const s3Days = {};
  myHud.filter(e => e.section === 'PAR-S3').forEach(e => {
    const day = parseInt((e.date || '').split('-')[2], 10);
    if (!day) return;
    s3Days[day] = (s3Days[day] || 0) + (e.hours || 0);
  });
  // Also pull from hudTrainingEntries (M = marketing)
  myTal.filter(e => e.certificationActivityType === 'M').forEach(e => {
    const day = parseInt((e.date || '').split('-')[2], 10);
    if (!day) return;
    s3Days[day] = (s3Days[day] || 0) + (parseFloat(e.durationHours || 0));
  });
  ...
  ws.getCell(S4_ROW, LABEL_COL).value = 'Non-HUD Hours (8 − HUD hours per day, min 0)';
  let s4Total = 0;
  for (let d = 1; d <= daysInMonth; d++) {
    const s1h = Object.values(s1DayTotals).reduce((t, days) => t + (days[d] || 0), 0);
    const s2h = s2Days[d] || 0;
    const s3h = s3Days[d] || 0;
    const hudHrs = s1h + s2h + s3h;
    if (hudHrs === 0) continue; // no entries → leave blank
    const nonHud = Math.max(0, 8 - hudHrs);
    if (nonHud > 0) { ws.getCell(S4_ROW, d + 1).value = nonHud; s4Total += nonHud; }
  }
  if (s4Total) ws.getCell(S4_ROW, TOTAL_COL).value = s4Total;
...
async function buildCML(counselor, data) {
  const { lastDayFmt, fileDateStr, clientMap, sessions, hudEntries } = data;

  // Manual CML entries (not auto-created from sessions)
  const manualEntries = hudEntries
    .filter(e => e.counselorId === counselor.id && e.section === 'CML' && !e.autoCreated);

  // Sessions where client is CM or Court billing, for this counselor
  const autoSessions = sessions.filter(s => {
    const c = clientMap[s.clientId];
    return s.counselor === counselor.name &&
      (c?.billingType === 'Case Management Activity' || c?.billingType === 'Court');
  });
  ...
  const rows = [
    ...manualEntries.map(e => ({
      date:    e.date,
      name:    counselor.name,
      desc:    e.activityDescription || '',
      minutes: roundTo15((e.hours || 0) * 60),
    })),
    ...autoSessions.map(s => ({
      date:    fmtDate(s.dateObj),
      name:    counselor.name,
      desc:    clientMap[s.clientId]?.clientName || s.clientId,
...
async function genTAL(data) {
  const { lastDayFmt, fileDateStr, talEntries, counselors } = data;

  const counselorMap = {};
  counselors.forEach(c => { counselorMap[c.id] = c; });

  const rows = [...talEntries].sort((a, b) => {
  ...
  rows.forEach((e, i) => {
    ...
    ws.getCell(r, 2).value = e.time    || '';
    ws.getCell(r, 3).value = c.staffNumber ?? e.staffNumber ?? '';
    ws.getCell(r, 4).value = e.counselorName || c.name || '';
    ws.getCell(r, 5).value = e.activityDescription || e.description || '';
    ws.getCell(r, 6).value = e.certificationActivityType || e.type || '';
    ws.getCell(r, 7).value = e.costType || '';
    ws.getCell(r, 8).value = parseFloat(e.durationHours || 0);
  });
...
async function genINV(data) {
  const { lastDayFmt, fileDateStr, counselors, hudEntries } = data;
  ...
  activeCounselors.forEach(c => {
    const myHud = hudEntries.filter(e => e.counselorId === c.id && (e.section || '').startsWith('PAR'));
    if (!myHud.length) return;
    const totalHrs    = myHud.reduce((s, e) => s + (e.hours || 0), 0);
...
async function genAll(data) {
  ...
  // CAL
  const calWb = newWB(); // re-use buildCAL logic via buffer
  const calBuf = await (async () => {
    // Build inline to avoid circular helper issue
    const { year, mon, lastDayFmt, clientMap, nofaRxMap, sessions } = data;
    const rows = sessions
      .filter(s => clientMap[s.clientId]?.billingType === 'In-Person' && nofaRxMap[s.clientId])
      .sort((a, b) => a.dateObj - b.dateObj);
    ...
    rows.forEach((s, i) => {
      const c = clientMap[s.clientId]; const r = dsr + 1 + i;
      ws.getCell(r,1).value = fmtDate(s.dateObj);
      ws.getCell(r,2).value = nofaRxMap[s.clientId] || '';
      ws.getCell(r,3).value = s.counselor || '';
      ws.getCell(r,4).value = c.firstName; ws.getCell(r,5).value = c.lastName;
      ws.getCell(r,6).value = 'In-Person'; ws.getCell(r,7).value = 'Under Progress';
      ws.getCell(r,8).value = roundTo15((parseFloat(s.hours)||0)*60);
    });
    return wb.xlsx.writeBuffer();
  })();
  ...
  data.talEntries.sort((a,b) => (a.date||'').localeCompare(b.date||'')).forEach((e, i) => {
    const c = cMap[e.counselorId] || {}; const r = 7 + i;
    talWs.getCell(r,1).value = e.date||''; talWs.getCell(r,2).value = e.time||'';
    talWs.getCell(r,3).value = c.staffNumber??''; talWs.getCell(r,4).value = e.counselorName||'';
    talWs.getCell(r,5).value = e.activityDescription||''; talWs.getCell(r,6).value = e.certificationActivityType||'';
    talWs.getCell(r,7).value = e.costType||''; talWs.getCell(r,8).value = parseFloat(e.durationHours||0);
  });
  ...
  active.forEach(c => {
    const hrs = data.hudEntries.filter(e=>e.counselorId===c.id&&(e.section||'').startsWith('PAR')).reduce((s,e)=>s+(e.hours||0),0);
    if (!hrs) return;
